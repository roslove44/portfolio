...
The timing irony is that this same Next.js release adds a **React Compiler written in Rust** inside Turbopack (more on that below). Everyone eventually leaves JavaScript to compile JavaScript, each with their favorite systems language.
...
It also works inside a `use cache` scope. For now it is limited to Server Components, with route handlers and Server Actions announced for a future release.

On a bilingual site like this one, it is the kind of detail that deletes a dozen useless props.
...
The main benefit: hot-module reloading works for Server Components that read local files. If you build an MDX blog from disk, you see the result without restarting.
...
Now, the part that cost me an evening.
...
On my machine sharp is present, since it is required for image optimization in standalone mode. So sharp does the rasterizing, and it has to **decode SVG** to produce the PNG.
...
The result: the image optimizer forbids SVG decoding to protect itself, and cuts the legs off the OG generator, which just became an SVG decoder. Two teams in the same codebase, one shared global, neither of them wrong in isolation.
