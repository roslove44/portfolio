...
Les chiffres publiés par Vercel sur leurs propres apps, après compilation de 50 routes :
...
L'ironie du calendrier, c'est que la même release Next.js ajoute un **React Compiler en Rust** côté Turbopack (voir plus bas). Tout le monde finit par quitter JavaScript pour compiler JavaScript, chacun avec son langage système préféré.
...
Ça marche aussi à l'intérieur d'un scope `use cache`. Pour l'instant c'est limité aux Server Components, le support des route handlers et des Server Actions est annoncé pour plus tard.

Sur un site bilingue comme celui-ci, c'est le genre de détail qui supprime une dizaine de props inutiles.
...
L'intérêt principal : le hot-module reloading fonctionne pour les Server Components qui lisent des fichiers locaux. Si tu construis un blog en MDX lu depuis le disque, tu vois le résultat sans redémarrer.
...
Maintenant, la partie qui m'a coûté une soirée.
...
if (sharp) {
	...
	const resvgJS = new resvg.Resvg(svg, { fitTo: { mode: 'width', value: options.width } });
	pngBuffer = Buffer.from(resvgJS.render().asPng());
}
```

Chez moi sharp est présent, puisqu'il est requis pour l'optimisation d'images en mode standalone. Donc c'est sharp qui rasterise, et il doit **décoder du SVG** pour produire le PNG.
...
Résultat : l'optimiseur d'images interdit le décodage SVG pour se protéger, et coupe au passage les jambes du générateur d'OG, qui vient justement de se mettre à décoder du SVG. Deux équipes de la même codebase, une variable globale partagée, aucune des deux n'a tort isolément.
